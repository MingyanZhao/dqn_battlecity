		...
		while self.running:
			...
			if dqn.DQN_TRAINING == True:
				if players[0].state == players[0].STATE_ALIVE:
					if frame_num > 16:
						if frame_num % 4 == 0:
							action_select_by_dqn = mydqn.choose_action(db.get_current_observation(), frame_num)[0]
			...
			db.save(frame_num, img_array, action, reward)
			...
			if frame_num > 1600 and frame_num % 8 == 0:
				training_start = time.clock()
				mydqn.dqn_training(db, frame_num)
			...
			if frame_num % 1000 == 0:
				print 'number of frames :' + str(frame_num) + ' frame round: ' + str(frame_round)
				...
				logger.log_write_info('frame num = ' + str(frame_num) + ' frame round: ' + str(frame_round))
