	...
	def getFreeSpawningPosition(self):
		...
		available_positions = [
			...
			[12 * self.level.TILE_SIZE + (self.level.TILE_SIZE * 2 - self.rect.width) / 2,
			 16 * self.level.TILE_SIZE + (self.level.TILE_SIZE * 2 - self.rect.height) / 2],
			[6 * self.level.TILE_SIZE + (self.level.TILE_SIZE * 2 - self.rect.width) / 2,
			 16 * self.level.TILE_SIZE + (self.level.TILE_SIZE * 2 - self.rect.height) / 2],
		]
...
class Game():
	...
	def nextLevel(self):
		...
		player_killed = 0
		tanks_stack = []
		enemy_in_same_position_count = 0


		while self.running:
			time_passed = self.clock.tick(50)
			img_array = ai.pygame_to_cvimage(screen.subsurface(0,0,416,416))


			if dqn.TEACHING == True:
				for event in pygame.event.get():
			...
			if dqn.TEACHING == False and dqn.DQN_TRAINING == True:

				if players[0].state == players[0].STATE_ALIVE:
					if frame_num > 16:
						if frame_num % 4 == 0:
							action_select_by_dqn = mydqn.choose_action(db.get_current_screen_data(), frame_num)[0]
			...
			for enemy in enemies:
				if enemy.state == enemy.STATE_DEAD and not self.game_over and self.active:
					...
					enemy_killed += 1
					logger.log_write_info(' ememy has been killed for ' + str(enemy_killed) + ' times ')
					if len(self.level.enemies_left) == 0 and len(enemies) == 0:
						if dqn.DQN_TRAINING == True:
							self.level.enemies_left = [0]
			...
			if not self.game_over and self.active:
				for player in players:
					if player.state == player.STATE_ALIVE:
						...
						reward = -255
						player_killed += 1
						logger.log_write_info('player has been killed for ' + str(player_killed) + ' times ')
			...
			if dqn.TEACHING == True:
				if frame_num > 1000:
					dqn.TEACHING = False
			elif frame_num > 16 and frame_num % 8 == 0:
				training_start = time.clock()
				mydqn.dqn_training()
				logger.log_write_info('training time = %d', time.clock() - training_start)
