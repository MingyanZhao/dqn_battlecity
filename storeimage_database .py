		...
		for i in range(minibatch_length):
			...
			while(action == -1):
				...
				if state_index >= len(actions) or state_index >= len(rewards):
					...
					print 'length of actions = ' + str(len(actions))
...
def add_action(a):
	print 'frame =' + str(current_frame_num) + ', record action ' + str(a) + ', actions recorded = ' + str(len(actions))
	actions.append(a)

def add_reward(r):
	print 'frame =' + str(current_frame_num) + ', record reward ' + str(r) + ', rewards recorded = ' + str(len(rewards))
	rewards.append(r)
